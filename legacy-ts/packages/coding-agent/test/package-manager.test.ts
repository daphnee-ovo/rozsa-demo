...
Content`,
		...
		it("should auto-discover root markdown skills from .rozsa skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .rozsa", async () => {
			const extDir = join(tempDir, ".rozsa", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".rozsa"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".rozsa", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".rozsa", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".rozsa", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".rozsa", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".rozsa", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json rozsa.extensions in extensions setting", async () => {
			// Create a package with rozsa.extensions in package.json
			...
			// Should find the extensions declared in package.json rozsa.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .rozsa/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-rozsa", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-rozsa"), { recursive: true });
		...
		it("should use the project .rozsa dir as baseDir for project .rozsa skills", async () => {
			const projectBaseDir = join(tempDir, ".rozsa");
			const skillPath = join(projectBaseDir, "skills", "project-rozsa", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-rozsa"), { recursive: true });
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".rozsa", "agent");
		...
		it("should dedupe user skill entries when ~/.rozsa/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .rozsa auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".rozsa\n");

			const skillDir = join(tempDir, ".rozsa", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with rozsa manifest", async () => {
		...
		it("should keep rozsa manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.rozsa\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".rozsa", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the rozsa-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("source parsing", () => {
		it("should emit progress events on install attempt", { timeout: 30000 }, async () => {
			const events: ProgressEvent[] = [];
			packageManager.setProgressCallback((event) => events.push(event));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .rozsa settings base", () => {
			...
			const rel = relative(join(tempDir, ".rozsa"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in rozsa manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json rozsa.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".rozsa", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".rozsa", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".rozsa", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".rozsa", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".rozsa", "npm", "node_modules", "example");

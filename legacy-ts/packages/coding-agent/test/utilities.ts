...
import { Agent } from "@earendil-works/rozsa-agent-core";
import { getModel, type OAuthCredentials, type OAuthProvider } from "@earendil-works/rozsa-ai";
...
// ============================================================================
// OAuth API key resolution from ~/.rozsa/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".rozsa", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.rozsa/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.rozsa/agent/auth.json
 */
...
/** Path to the real rozsa agent config directory */
export const ROZSA_AGENT_DIR = join(homedir(), ".rozsa", "agent");

/**
 * Get an AuthStorage instance backed by ~/.rozsa/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `rozsa-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);

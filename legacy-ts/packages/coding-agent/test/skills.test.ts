...
describe("skills", () => {
	...
	describe("formatSkillsForPrompt", () => {
		...
		it("should include intro text before XML", () => {
			...
			expect(introText).toContain("Only when the task matches and the content has not already been provided inline");
		});
	...
	describe("loadSkills with options", () => {
		...
		it("should expand ~ in skillPaths", () => {
			const homeSkillsDir = join(homedir(), ".rozsa/agent/skills");
			const { skills: withTilde } = loadSkills({
				agentDir: emptyAgentDir,
				cwd: emptyCwd,
				skillPaths: ["~/.rozsa/agent/skills"],
				includeDefaults: true,
			});

...
describe("regression #2860: replaced session callbacks", () => {
	...
	async function createRuntimeForTest(extensionFactory: ExtensionFactory, responses: string[]) {
		const tempDir = join(tmpdir(), `rozsa-2860-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("rebinds before withSession, targets the replacement session, and invalidates stale rozsa/ctx", async () => {
		const events: string[] = [];
		...
		const { runtime } = await createRuntimeForTest(
			(rozsa) => {
				...
				rozsa.on("session_start", () => {
					events.push(`start:${currentInstance}`);
				});
				rozsa.on("session_shutdown", () => {
					events.push(`shutdown:${currentInstance}`);
				});
				rozsa.registerCommand("repro", {
					description: "repro",
	...
	it("supports withSession for switchSession", async () => {
		...
		const { runtime } = await createRuntimeForTest(
			(rozsa) => {
				rozsa.registerCommand("switch-it", {

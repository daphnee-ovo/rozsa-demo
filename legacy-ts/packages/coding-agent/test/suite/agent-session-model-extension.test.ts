import type { AgentTool, ThinkingLevel } from "@earendil-works/rozsa-agent-core";
import { fauxAssistantMessage, fauxToolCall, type Model } from "@earendil-works/rozsa-ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("setModel saves the model and emits model_select", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(rozsa) => {
					rozsa.on("model_select", async (event) => {
	...
	it("allows extension tool_call handlers to block tool execution", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(rozsa) => {
					rozsa.on("tool_call", async () => ({ block: true, reason: "Blocked by test" }));
				},
			],
		});
		harnesses.push(harness);
	...
	it("bindExtensions emits session_start and reload emits session_shutdown then session_start", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(rozsa) => {
					rozsa.on("session_start", async (event) => {
						lifecycleEvents.push(`start:${event.reason}`);
					});
					rozsa.on("session_shutdown", async (event) => {

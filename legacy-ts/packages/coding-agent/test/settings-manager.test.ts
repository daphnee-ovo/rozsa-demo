...
describe("SettingsManager", () => {
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates rozsa starting up)
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".rozsa", "settings.json");
	...
	describe("project settings directory creation", () => {
		it("should not create .rozsa folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .rozsa folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .rozsa folder that beforeEach created
			rmSync(join(projectDir, ".rozsa"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .rozsa folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".rozsa"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .rozsa folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .rozsa folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .rozsa folder should NOT exist yet
			expect(existsSync(join(projectDir, ".rozsa"))).toBe(false);
			...
			// Now .rozsa folder should exist
			expect(existsSync(join(projectDir, ".rozsa"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".rozsa", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".rozsa", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".rozsa", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);

...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "rozsa-ext-test-"));
	...
	const extensionCode = `
		export default function(rozsa) {
			rozsa.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers subdirectory with package.json rozsa field", async () => {
	...
	it("keeps package.json rozsa extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with rozsa field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without rozsa field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ rozsa: { extensions: ["./entry.ts"] } }));
	...
	it("registers message renderers", async () => {
		const extCode = `
			export default function(rozsa) {
				rozsa.registerMessageRenderer("my-custom-type", (message, options, theme) => {
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(rozsa) {
				rozsa.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(rozsa) {
				rozsa.on("agent_start", async () => {});
				rozsa.on("tool_call", async (event) => undefined);
				rozsa.on("agent_end", async () => {});
			}
		`;

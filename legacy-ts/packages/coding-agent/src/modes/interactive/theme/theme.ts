...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		const highContrastPath = path.join(themesDir, "high-contrast.json");
		BUILTIN_THEMES = {
			dark: JSON.parse(fs.readFileSync(darkPath, "utf-8")) as ThemeJson,
			light: JSON.parse(fs.readFileSync(lightPath, "utf-8")) as ThemeJson,
			"high-contrast": JSON.parse(fs.readFileSync(highContrastPath, "utf-8")) as ThemeJson,
		};
	}
...
const THEME_KEY = Symbol.for("@earendil-works/rozsa-coding-agent:theme");
const THEME_KEY_OLD = Symbol.for("@mariozechner/rozsa-coding-agent:theme");

...
class TreeList implements Component {
	...
	render(width: number): string[] {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			// Node marker - git graph style
			const isOnActivePath = this.activePathIds.has(entry.id);
			const isCurrentLeaf = entry.id === this.currentLeafId;
			const hasMultipleChildren = (this.visibleChildrenMap.get(entry.id) ?? []).length > 1;
			const nodeMarker = isCurrentLeaf
				? theme.fg("accent", "◉ ")
				: isOnActivePath
					? theme.fg("accent", "● ")
					: hasMultipleChildren
			...
			let line = cursor + theme.fg("dim", prefix) + foldMarker + nodeMarker + label + labelTimestamp + content;
			if (isSelected) {

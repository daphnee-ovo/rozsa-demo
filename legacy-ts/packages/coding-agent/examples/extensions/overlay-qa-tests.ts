 ...
 *
 * Usage: rozsa --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@earendil-works/rozsa-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@earendil-works/rozsa-tui";
import { matchesKey, truncateToWidth, visibleWidth } from "@earendil-works/rozsa-tui";
...
export default function (rozsa: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like rozsa-doom would need)
	rozsa.registerCommand("overlay-animation", {
	...
	rozsa.registerCommand("overlay-anchors", {
	...
	rozsa.registerCommand("overlay-margins", {
	...
	rozsa.registerCommand("overlay-stack", {
	...
	rozsa.registerCommand("overlay-overflow", {
	...
	rozsa.registerCommand("overlay-edge", {
	...
	rozsa.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	rozsa.registerCommand("overlay-maxheight", {
	...
	rozsa.registerCommand("overlay-sidepanel", {
	...
	rozsa.registerCommand("overlay-toggle", {
	...
	rozsa.registerCommand("overlay-passive", {
	...
	rozsa.registerCommand("overlay-focus", {
	...
	rozsa.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/rozsa-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/rozsa-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like rozsa-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (rozsa-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));

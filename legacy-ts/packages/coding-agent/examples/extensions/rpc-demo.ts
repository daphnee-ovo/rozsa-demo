...
export default function (rozsa: ExtensionAPI) {
	...
	rozsa.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "rozsa RPC Demo (new session)" : "rozsa RPC Demo");
	...
	rozsa.on("session_before_switch", async (event, ctx) => {
	...
	rozsa.registerCommand("rpc-input", {
	...
	rozsa.registerCommand("rpc-editor", {
	...
	rozsa.registerCommand("rpc-prefill", {

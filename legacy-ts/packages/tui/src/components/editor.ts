...
export class Editor implements Component, Focusable {
	...
	// 顶部边框内嵌标题（如 bash 模式提示）
	public borderTitle: string | undefined = undefined;
	...
	render(width: number): string[] {
		...
		if (this.scrollOffset > 0) {
			...
			if (remaining >= 0) {
				...
				result.push(this.borderColor(`╭${truncateToWidth(indicator, innerWidth)}╮`));
			}
		} else if (this.borderTitle) {
			const title = ` ${this.borderTitle} `;
			const titleWidth = visibleWidth(title);
			const remaining = innerWidth - titleWidth;
			if (remaining >= 0) {
				result.push(
					this.borderColor("╭") +
						this.borderColor("─") +
						title +
						this.borderColor(`${"─".repeat(remaining - 1)}╮`),
				);
			} else {
		...
		for (const layoutLine of visibleLines) {
			...
			// 带左右竖线的内容行
			result.push(`${vBorder}${leftPadding}${displayText}${padding}${lineRightPadding}${vBorder}`);
		}
		...
		// Add autocomplete list if active (outside border box, indented to align)

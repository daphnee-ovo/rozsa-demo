...
const outputPath = join(tmpdir(), "rozsa-browser-smoke.js");
const errorLogPath = join(tmpdir(), "rozsa-browser-smoke-errors.log");

try {
	await build({
		entryPoints: ["devtools/before/browser-smoke-entry.ts"],
		bundle: true,
		platform: "browser",
		format: "esm",
		logLevel: "silent",

# @earendil-works/rozsa-agent-core

Stateful agent with tool execution and event streaming. Built on `@earendil-works/rozsa-ai`.

## Installation

```bash
npm install @earendil-works/rozsa-agent-core
```
...
```typescript
declare module "@earendil-works/rozsa-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@earendil-works/rozsa-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@earendil-works/rozsa-agent-core";

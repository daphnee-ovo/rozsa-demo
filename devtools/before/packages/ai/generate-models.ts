...
async function generateModels() {
	...
	for (const candidate of allModels) {
		...
		// Claude 4.6+ 模型同时支持 200K 和 1M 上下文模式
		const claudeContextModesIds = [
			"claude-opus-4-6", "claude-opus-4-7", "claude-sonnet-4-6",
			"claude-opus-4.6", "claude-opus-4.7", "claude-sonnet-4.6",
		];
		const claudeContextModesPatterns = [
			/^(us\.|eu\.|au\.|global\.|jp\.)?anthropic\.claude-(opus|sonnet)-4-[67]/,
			/^anthropic\/claude-(opus|sonnet)-4\.[567]/,
		];
		const matchesContextModes = claudeContextModesIds.includes(candidate.id) ||
			claudeContextModesPatterns.some((p) => p.test(candidate.id));
		if (matchesContextModes && candidate.contextWindow >= 1000000) {
			candidate.contextModes = [200000, 1000000];
		}
	}
...
`;
			...
			if (model.contextModes) {
				output += `\t\t\tcontextModes: ${JSON.stringify(model.contextModes)},\n`;
			}

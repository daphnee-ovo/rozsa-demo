...
```bash
npm install @earendil-works/rozsa-ai
```

TypeBox exports are re-exported from `@earendil-works/rozsa-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@earendil-works/rozsa-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@earendil-works/rozsa-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@earendil-works/rozsa-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@earendil-works/rozsa-ai';
...
```typescript
import { getImageModel, generateImages } from '@mariozechner/rozsa-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@earendil-works/rozsa-ai';
...
```typescript
import { getModel, stream } from '@earendil-works/rozsa-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@earendil-works/rozsa-ai';
...
```typescript
import { Model, stream } from '@earendil-works/rozsa-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@earendil-works/rozsa-ai';
...
```typescript
import { getModel, complete, Context } from '@earendil-works/rozsa-ai';
...
```typescript
import { Context, getModel, complete } from '@earendil-works/rozsa-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@earendil-works/rozsa-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@earendil-works/rozsa-ai';
...
```bash
npx @earendil-works/rozsa-ai login              # interactive provider selection
npx @earendil-works/rozsa-ai login anthropic    # login to specific provider
npx @earendil-works/rozsa-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@earendil-works/rozsa-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@earendil-works/rozsa-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@earendil-works/rozsa-ai/oauth';
...
```typescript
import { getModel, complete } from '@earendil-works/rozsa-ai';
import { getOAuthApiKey } from '@earendil-works/rozsa-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@earendil-works/rozsa-ai`
- Add credential detection in `env-api-keys.ts` for the new provider
- Ensure `streamSimple` handles auth lookup via `getEnvApiKey()` or provider-specific auth

#### 4. Model Generation (`devtools/before/packages/ai/generate-models.ts`, `devtools/before/packages/ai/generate-image-models.ts`)

- Add logic to fetch and parse models from the provider's source (e.g., models.dev API)
- Map chat/tool-capable provider model data to the standardized `Model` interface via `devtools/before/packages/ai/generate-models.ts`
- Map image-generation provider model data to the standardized `ImagesModel` interface via `devtools/before/packages/ai/generate-image-models.ts`

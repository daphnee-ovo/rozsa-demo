...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @earendil-works/rozsa-ai <command> [provider]
...
Examples:
  ...
  npx @earendil-works/rozsa-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @earendil-works/rozsa-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @earendil-works/rozsa-ai --help' for usage`);
	process.exit(1);
}

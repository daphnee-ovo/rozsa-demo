...
describe("Editor component", () => {
	...
	describe("Grapheme-aware text wrapping", () => {
		...
		it("wraps CJK characters correctly (each is 2 columns wide)", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const width = 10 + 1 + 2; // +1 cursor +2 side borders
			...
			// Verify content split correctly (strip borders + ANSI + trim)
			const contentLines = lines.slice(1, -1).map((l) => stripVTControlCharacters(l).replace(/^│|│$/g, "").trim());
		...
		it("handles mixed ASCII and wide characters in wrapping", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme);
			const width = 15 + 1 + 2; // +1 cursor +2 side borders
		...
		it("shows cursor at end of line before wrap, wraps on next char", () => {
			const width = 12; // 9 content + 1 cursor + 2 borders
			for (const paddingX of [0, 1]) {
				...
				// Cursor (reverse video space) should appear before right border
				assert.ok(contentLines[0]!.includes("\x1b[7m \x1b[0m"), "Cursor should be visible");
	...
	describe("Word wrapping", () => {
		it("wraps at word boundaries instead of mid-word", () => {
			...
			// Get content lines (between borders), strip ANSI + border chars + trim
			const contentLines = lines.slice(1, -1).map((l) => stripVTControlCharacters(l).replace(/^│|│$/g, "").trim());
	...
	describe("Sticky column", () => {
		...
		it("handles editor resizes when preferredVisualCol is on the same line", () => {
			...
			// Render with narrower width to simulate resize (+2 for side borders)
			editor.render(14); // layoutWidth = 14 - 2 - 1 = 11
		...
		it("handles editor resizes when preferredVisualCol is on a different line", () => {
			...
			// Create a line that wraps into multiple visual lines at width 12 (layoutWidth = 9)
			// "12345678901234567890" = 20 chars, wraps to 2+ visual lines at layoutWidth 9
			editor.setText("short\n12345678901234567890");
			...
			// Narrow the editor (+2 for side borders)
			editor.render(12);

			// Move down - preferredVisualCol was 15, but layoutWidth is 9
			// Should land on line 1, clamped to width (visual col 8)
		...
		it("rewrapped lines: target fits current visual column", () => {
			...
			// Narrow to width 12 (layoutWidth = 9, +2 for side borders).
		...
		it("rewrapped lines: target shorter than current visual column", () => {
			...
			// Narrow to width 12 (layoutWidth = 9, +2 for side borders). Moving down clamps to col 8
			editor.render(12);
			editor.handleInput("\x1b[B");
			assert.deepStrictEqual(editor.getCursor(), { line: 1, col: 8 });
	...
	describe("Paste marker atomic behavior", () => {
		...
		it("does not get stuck moving down from a multi-visual-line paste marker", () => {
			// +2 for side borders: render(22) → layoutWidth = 19
			const tui = createTestTUI(22, 24);
			const editor = new Editor(tui, defaultEditorTheme);
			...
			//
			// Marker "[paste #1 +100 lines]" (21 chars) is wider than
			// layoutWidth (19). Word-wrap splits at the space before "lines",
			// producing:
			...
			assert.ok(markerLen > 19, "marker should be wider than layoutWidth");
			const markerStart = 8;

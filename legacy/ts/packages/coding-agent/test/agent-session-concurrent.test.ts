...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `rozsa-concurrent-test-${Date.now()}`);
	...
	function createSession() {
		...
		const settingsManager = SettingsManager.inMemory({ permissionMode: "free-permission" });
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(rozsa) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = rozsa;
			},
			(rozsa) => {
		...
		expect(rozsa).toBeDefined();

		rozsa!.sendUserMessage("Steer from extension", { deliverAs: "steer" });

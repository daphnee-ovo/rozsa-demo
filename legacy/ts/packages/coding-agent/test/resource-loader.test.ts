...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "rozsa-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".rozsa", "prompts");
			...
			const projectSkillDir = join(cwd, ".rozsa", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".rozsa", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".rozsa", "themes"), { recursive: true });
		...
		it("should load symlinked user and project extensions once", async () => {
			...
			writeFileSync(
				...
				`export default function(rozsa) {
	rozsa.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".rozsa"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".rozsa", "extensions"), "dir");

			const loader = new DefaultResourceLoader({ cwd, agentDir });
			await loader.reload();

			const extensionsResult = loader.getExtensions();
			// +1 for builtin output filter (appended after file-loaded extensions)
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".rozsa", "extensions", "shared.ts"));
		});

		it("should keep both extensions loaded when command names collide", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".rozsa", "extensions");
			...
			writeFileSync(
				...
				`export default function(rozsa) {
	rozsa.registerCommand("deploy", {
		description: "project deploy",
		handler: async () => {},
	});
	rozsa.registerCommand("project-only", {
	...
	rozsa.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.rozsa", async () => {
			const piDir = join(cwd, ".rozsa");

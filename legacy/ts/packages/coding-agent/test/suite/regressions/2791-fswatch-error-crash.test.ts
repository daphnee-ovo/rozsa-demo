...
/**
 * Regression test for https://github.com/earendil-works/rozsa-mono/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "rozsa-2791-"));
...
process.env.ROZSA_CODING_AGENT_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync(process.execPath, [scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, ROZSA_CODING_AGENT_DIR: agentDir },

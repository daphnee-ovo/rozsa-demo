...
describe("Documentation example", () => {
	it("custom compaction example should type-check correctly", () => {
		...
		const exampleExtension = (rozsa: ExtensionAPI) => {
			rozsa.on("session_before_compact", async (event: SessionBeforeCompactEvent, ctx) => {
	...
	it("compact event should have correct fields", () => {
		const checkCompactEvent = (rozsa: ExtensionAPI) => {
			rozsa.on("session_compact", async (event: SessionCompactEvent) => {

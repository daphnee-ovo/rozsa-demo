...
function parseArgs(argv: string[]): Args {
	...
	let sessionPath = resolve(join(tmpdir(), `rozsa-sdk-codex-cache-probe-tool-loop-${Date.now()}.jsonl`));
...
function buildPrompt(turn: number): string {
	...
	const lines: string[] = [];
	lines.push("This is an automated prompt-cache tool-loop test executed through the rozsa coding-agent SDK.");

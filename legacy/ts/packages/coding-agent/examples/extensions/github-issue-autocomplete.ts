...
async function resolveGitHubRepo(rozsa: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await rozsa.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (rozsa: ExtensionAPI): void {
	rozsa.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(rozsa, ctx.cwd);
		if (!resolvedRepo.ok) {

...
async function checkDirtyRepo(
	...
	const { stdout, code } = await rozsa.exec("git", ["status", "--porcelain"]);
...
export default function (rozsa: ExtensionAPI) {
	rozsa.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(rozsa, ctx, action);
	});

	rozsa.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(rozsa, ctx, "fork");
	});
}

 ...
 *
 * Demonstrates rozsa.sendUserMessage() for sending user messages from extensions.
 * Unlike rozsa.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (rozsa: ExtensionAPI) {
	...
	rozsa.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			rozsa.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	rozsa.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				rozsa.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	rozsa.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				rozsa.sendUserMessage(args, { deliverAs: "followUp" });

...
fn non_macos_materializes_the_existing_css_sidebar_and_settings_navigation() {
    ...
    assert!(css.contains("grid-template-columns: var(--main-sidebar-width) minmax(0, 1fr)"));
    assert!(css.contains("grid-template-columns: var(--settings-sidebar-width) minmax(0, 1fr)"));
...
fn macos_main_webview_uses_native_pane_width_and_the_existing_sidebar_webview_overlay() {
    ...
    assert!(css.contains("body.native-split-main [data-od-id=\"app-body\"]"));
    assert!(css.contains("display: block;"));
    assert!(css.contains("body.native-split-main [data-od-id=\"main-panel\"]"));

...
fn native_collapsed_sidebar_reveals_at_the_edge_and_avoids_the_titlebar() {
    ...
    assert!(frontend.contains("if (nativeSidebarOverlayVisible) hideNativeSidebarOverlay();"));
    ...
    assert!(
        sidebar_frontend
            .contains("sidebarInvoke('set_native_sidebar_overlay_visible', { visible: false })")
    );

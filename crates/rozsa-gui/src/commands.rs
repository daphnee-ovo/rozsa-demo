    ...
    let agent = match tab {
        ...
        SessionTab::Loaded { path, messages } => {
            ...
            *tab = SessionTab::Active {
                ...
                live: crate::state::LiveState { messages: std::mem::take(messages), ..Default::default() },
            };
            // 启动事件转发
            drop(tabs);
        ...
        SessionTab::Idle { path, .. } => {
            ...
            *tab = SessionTab::Active {
                path: path_owned,
                agent: agent_arc.clone(),
                live: crate::state::LiveState { messages, ..Default::default() },
            };
            drop(tabs);
            crate::events::spawn_event_forwarder_for_tab(app.clone(), idx, state.inner().clone());
            agent_arc
...
#[tauri::command]
pub async fn switch_session(state: State<'_, GuiState>, app: AppHandle, path: String) -> Result<(), String> {
    let mut tabs = state.tabs.lock().await;
    ...
    let target_idx = if let Some(idx) = existing_idx {
        ...
        tabs.push(SessionTab::Loaded { path: path.clone(), messages });
        tabs.len() - 1
    };
...
pub async fn new_session(state: State<'_, GuiState>, app: AppHandle) -> Result<String, String> {
    ...
    let mut tabs = state.tabs.lock().await;
    tabs.push(SessionTab::Loaded { path: path_str.clone(), messages: vec![] });
...
#[tauri::command]
pub async fn rename_session(state: State<'_, GuiState>, path: String, name: String) -> Result<(), String> {
...
pub async fn run_bash(
    ...
    let shared = state.shared.clone();
    let tabs_ref = state.tabs.clone();
    let active_ref = state.active_tab.clone();
    tokio::spawn(async move {

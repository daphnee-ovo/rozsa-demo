...
use rozsa_model::types::{
    Api, AssistantMessage, ContentBlock, Message, Provider, StopReason, Usage, UsageCost,
...
async fn full_conversation_flow_with_mock_backend() {
    ...
    let streaming_state = NativeUiState {
        is_streaming: true,
        messages: vec![user_msg("hello world"), assistant_msg("Hi")],
        ..state_msg.clone()
    };
    ...
    let done_state = NativeUiState {
        is_streaming: false,
        messages: vec![user_msg("hello world"), assistant_msg("Hi there! Done.")],
        ..state_msg
    };

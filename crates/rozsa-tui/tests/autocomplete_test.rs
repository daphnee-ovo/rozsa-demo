...
// --- 快速输入 prefix 过期回归 (ISSUE-I004) ---

#[test]
fn apply_completion_stale_prefix_slash_command() {
    // 面板打开时 prefix="/h"，但用户继续输入到 "/hel" 才按 Tab。
    // state.prefix 仍是旧值 "/h"，apply_completion 应正确替换整个 "/hel"。
    let items = vec![item("help", "/help", Some("Show help"))];
    let state = AutocompleteState::new("/h".to_string(), items);
    let (result, cursor) = apply_completion("/hel", 4, &state);
    assert_eq!(result, "/help ");
...
#[test]
fn apply_completion_stale_prefix_longer_input() {
    // 极端情况：prefix="/l"，但实际已输入 "/login"
    let items = vec![item("login", "/login", None)];
    let state = AutocompleteState::new("/l".to_string(), items);
    let (result, cursor) = apply_completion("/login", 6, &state);
    assert_eq!(result, "/login ");
    assert_eq!(cursor, 7);
}

#[test]
fn apply_completion_stale_prefix_at_mention() {
    // @ 路径补全：prefix="@s"，但用户已输入 "@src/m"
    let items = vec![item("src/main.rs", "src/main.rs", None)];
    let state = AutocompleteState::new("@s".to_string(), items);
    let (result, cursor) = apply_completion("read @src/m", 11, &state);

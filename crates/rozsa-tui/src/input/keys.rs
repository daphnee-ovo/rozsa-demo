...
pub fn handle_key(
    ...
    if let Some(ac_state) = state.autocomplete.take() {
        ...
        match action {
            ...
            crate::panels::autocomplete::AutocompleteAction::Close => {
                return Ok(());
            }
            crate::panels::autocomplete::AutocompleteAction::KeepOpen => {
                // Up/Down 导航或普通字符继续输入
    ...
    match key.code {
        ...
        KeyCode::Tab => {
            ...
            if is_autocomplete_context(&text, cursor) {
                crate::panels::autocomplete::request_autocomplete(&text, cursor, writer)?;
            }
        }
        KeyCode::Char(ch) => {
            // 有选区时先删除选区内容

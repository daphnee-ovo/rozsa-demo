...
pub fn render_sidebar(frame: &mut ratatui::Frame<'_>, area: Rect, state: &AppState) {
    let ui = &state.ui;
    ...
    append_agents(&mut lines, runtime, inner);
...
fn append_agents(lines: &mut Vec<Line<'static>>, runtime: Option<&Value>, inner: usize) {
    let Some(agents) = runtime
        .and_then(|state| state.pointer("/activeSubagents"))
    ...
    let viewing = runtime
        .and_then(|state| state.pointer("/viewingSubagentId"))
        .and_then(Value::as_str);
    lines.push(Line::raw(""));
    ...
    for agent in agents.iter().take(5) {
        ...
        lines.push(Line::raw(format!(
            "{icon} {} {}",
            truncate(name, inner.saturating_sub(elapsed.len() + 4)),
            elapsed
        )));
    }
    if agents.len() > 5 {

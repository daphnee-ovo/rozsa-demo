/**
 * Tool argument validation for Agent tool calls.
 *
 * Structure:
 * - validateToolArguments(): validates and coerces tool call arguments.
 * - JSON schema coercion helpers: preserve compatibility for plain JSON schemas.
 *
...
function isValidatorSchema(value: unknown): value is AgentTool["parameters"] {
	return isRecord(value);
}
...
function getValidator(schema: AgentTool["parameters"]): ReturnType<typeof Compile> {
...
export function validateToolArguments(tool: AgentTool, toolCall: AgentToolCall): unknown {

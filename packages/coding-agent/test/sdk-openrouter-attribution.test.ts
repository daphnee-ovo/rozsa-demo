...
describe("createAgentSession OpenRouter attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.ROZSA_TELEMETRY;
		delete process.env.ROZSA_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.ROZSA_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://rozsa.dev");
		expect(headers?.["X-OpenRouter-Title"]).toBe("rozsa");
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("rozsa");
	});

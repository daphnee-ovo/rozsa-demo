...
import { Agent, type AgentEvent, type AgentTool } from "@earendil-works/rozsa-agent-core";
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@earendil-works/rozsa-ai";
...
describe("AgentSession retry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `rozsa-retry-test-${Date.now()}`);

...
import { createAssistantMessageEventStream } from "@earendil-works/rozsa-ai";
...
export interface FauxStreamFnState {
	...
	callCount: number;
	/** The model passed to each call, in order. */
	models: Model<any>[];
	/** The context passed to each call, in order. */
	contexts: Context[];
	/** The stream options passed to each call, in order. */
	options: Array<SimpleStreamOptions | undefined>;
}
...
export function createFauxStreamFn(responses: FauxResponseInput[]): {
	...
	const state: FauxStreamFnState = { callCount: 0, models: [], contexts: [], options: [] };

	const streamFn = (model: Model<any>, context: Context, options?: SimpleStreamOptions) => {
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `rozsa-harness-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
function createHarnessWithResourceLoader(
	...
	settingsManager.applyOverrides({ permissionMode: "free-permission" });

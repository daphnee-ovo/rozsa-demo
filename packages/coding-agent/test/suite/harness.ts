...
import type { AgentMessage, AgentTool } from "@earendil-works/rozsa-agent-core";
import { Agent } from "@earendil-works/rozsa-agent-core";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@earendil-works/rozsa-ai";
import { registerFauxProvider } from "@earendil-works/rozsa-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `rozsa-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
export async function createHarness(options: HarnessOptions = {}): Promise<Harness> {
	...
	const settingsManager = SettingsManager.inMemory({ permissionMode: "free-permission", ...options.settings });

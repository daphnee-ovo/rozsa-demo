...
import { fauxAssistantMessage, registerFauxProvider } from "@earendil-works/rozsa-ai";
...
describe("AgentSessionRuntime characterization", () => {
	...
	async function createRuntimeForTest(
		...
		const tempDir =
			options?.cwd ?? join(tmpdir(), `rozsa-runtime-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const runtimeOptions = {
			...
			resourceLoaderOptions: {
				extensionFactories: [
					(rozsa: ExtensionAPI) => {
						rozsa.registerProvider(faux.getModel().provider, {
	...
	it("persists message_end assistant replacements to the session manager", async () => {
		const { runtime } = await createRuntimeForTest((rozsa: ExtensionAPI) => {
			rozsa.on("message_end", (event) => {
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtime } = await createRuntimeForTest((rozsa: ExtensionAPI) => {
			rozsa.on("session_before_switch", (event) => {
				events.push(event);
			});
			rozsa.on("session_shutdown", (event) => {
				events.push(event);
			});
			rozsa.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("honors session_before_switch cancellation for new and resume", async () => {
		...
		events.length = 0;
		const otherDir = join(tmpdir(), `rozsa-runtime-other-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtime } = await createRuntimeForTest((rozsa: ExtensionAPI) => {
			rozsa.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
	...
	it("duplicates the current active branch in-memory when forking at the current position", async () => {
		const tempDir = join(tmpdir(), `rozsa-runtime-suite-in-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		mkdirSync(tempDir, { recursive: true });
	...
	it("updates the runtime session cwd on cross-cwd session replacement", { timeout: 30000 }, async () => {
		const firstDir = join(tmpdir(), `rozsa-runtime-cwd-a-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		const secondDir = join(tmpdir(), `rozsa-runtime-cwd-b-${Date.now()}-${Math.random().toString(36).slice(2)}`);

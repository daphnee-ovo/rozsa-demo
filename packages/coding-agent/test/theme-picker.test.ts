...
describe("theme picker", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "rozsa-theme-picker-"));
		const agentDir = join(tempRoot, "agent");
		vi.stubEnv("ROZSA_CODING_AGENT_DIR", agentDir);
	...
	it("uses custom theme content names instead of file names", () => {
		...
		const themePath = join(process.env.ROZSA_CODING_AGENT_DIR!, "themes", "foo.json");

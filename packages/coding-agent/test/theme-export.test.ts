...
describe("getThemeExportColors", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "rozsa-theme-export-"));
		previousAgentDir = process.env.ROZSA_CODING_AGENT_DIR;
		process.env.ROZSA_CODING_AGENT_DIR = join(tempRoot, "agent");
		mkdirSync(join(process.env.ROZSA_CODING_AGENT_DIR, "themes"), { recursive: true });
	});

	afterEach(() => {
		rmSync(tempRoot, { recursive: true, force: true });
		if (previousAgentDir === undefined) {
			delete process.env.ROZSA_CODING_AGENT_DIR;
		} else {
			process.env.ROZSA_CODING_AGENT_DIR = previousAgentDir;
		}
	});
	...
	it("resolves export variable references using the same syntax as colors", () => {
		...
		writeFileSync(
			join(process.env.ROZSA_CODING_AGENT_DIR!, "themes", "custom-export-vars.json"),
	...
	it("resolves recursive vars and converts 256-color export values to hex", () => {
		...
		writeFileSync(
			join(process.env.ROZSA_CODING_AGENT_DIR!, "themes", "custom-export-recursive.json"),
			JSON.stringify(customTheme, null, 2),
		);

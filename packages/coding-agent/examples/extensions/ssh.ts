 ...
 *
 * Usage:
 *   rozsa -e ./ssh.ts --ssh user@host
 *   rozsa -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (rozsa: ExtensionAPI) {
	rozsa.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	rozsa.on("session_start", async (_event, ctx) => {
		...
		const arg = rozsa.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {

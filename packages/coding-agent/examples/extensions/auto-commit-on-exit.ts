...
export default function (rozsa: ExtensionAPI) {
	rozsa.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await rozsa.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[rozsa] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await rozsa.exec("git", ["add", "-A"]);
		const { code: commitCode } = await rozsa.exec("git", ["commit", "-m", commitMessage]);

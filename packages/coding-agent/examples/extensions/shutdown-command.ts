 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit rozsa cleanly.
 */
...
export default function (rozsa: ExtensionAPI) {
	// Register a /quit command that cleanly exits rozsa
	rozsa.registerCommand("quit", {
		description: "Exit rozsa cleanly",
	...
	rozsa.registerTool({
		...
		description: "Complete a task and exit rozsa",
	...
	rozsa.registerTool({
		...
		description: "Deploy the application and exit rozsa",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await rozsa.exec("npm", ["run", "deploy", params.environment], { signal });

...
import { Container, Loader, Spacer, Text, type TUI } from "@earendil-works/rozsa-tui";
import {
	DEFAULT_MAX_BYTES,
	DEFAULT_MAX_LINES,
...
export class BashExecutionComponent extends Container {
	...
	private updateDisplay(): void {
		...
		// Output
		const outputColor = this.status === "error" ? "error" : "muted";
		if (availableLines.length > 0) {
			if (this.isExpanded()) {
				// Show all lines
				const displayText = availableLines.map((line) => theme.fg(outputColor, line)).join("\n");
				this.contentContainer.addChild(new Text(`\n${displayText}`, 1, 0));
			} else {
				// Use shared visual truncation utility with width-aware caching
				const styledOutput = previewLogicalLines.map((line) => theme.fg(outputColor, line)).join("\n");
				const styledInput = `\n${styledOutput}`;

...
import { getLatestRozsaRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
Options:
  -l, --local    Install project-locally (.rozsa/settings.json)
...
Options:
  -l, --local    Remove from project settings (.rozsa/settings.json)
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "rozsa";
			if (sourceIsSelf) {
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		const latestRelease = await getLatestRozsaRelease(VERSION);

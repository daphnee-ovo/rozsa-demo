...
import type { ImageContent, Model } from "@earendil-works/rozsa-model-types";
...
export interface ExtensionRunnerOptions {
	/** 是否启用扩展 verbose 日志（加载、事件、错误等生命周期信息） */
	verbose?: boolean;
}
...
export class ExtensionRunner {
	...
	constructor(
		...
		// verbose 模式下输出已加载的扩展列表
		if (this.verbose) {
			for (const ext of extensions) {
				this.logVerbose(`Loaded: ${ext.path} (${ext.resolvedPath})`);
			}
		}
	}
	...
	private logVerbose(message: string): void {
		if (this.verbose) {
			console.error(`[ext:verbose] ${message}`);
		}
	}
	...
	emitError(error: ExtensionError): void {
		this.logVerbose(`Error in ${error.extensionPath}: ${error.error}`);
	...
	async emit<TEvent extends RunnerEmitEvent>(event: TEvent): Promise<RunnerEmitResult<TEvent>> {
		this.logVerbose(`Event: ${event.type}`);
		...
		for (const ext of this.extensions) {
			...
			for (const handler of handlers) {
				try {
					...
					this.logVerbose(`Error in ${ext.path}: ${message}`);
					this.emitError({
	...
	async emitMessageEnd(event: MessageEndEvent): Promise<AgentMessage | undefined> {
		this.logVerbose(`Event: message_end`);
		const ctx = this.createContext();
	...
	async emitToolResult(event: ToolResultEvent): Promise<ToolResultEventResult | undefined> {
		this.logVerbose(`Event: tool_result`);
		const ctx = this.createContext();
	...
	async emitToolCall(event: ToolCallEvent): Promise<ToolCallEventResult | undefined> {
		this.logVerbose(`Event: tool_call`);
		const ctx = this.createContext();
	...
	async emitUserBash(event: UserBashEvent): Promise<UserBashEventResult | undefined> {
		this.logVerbose(`Event: user_bash`);
		const ctx = this.createContext();
	...
	async emitBeforeProviderRequest(payload: unknown): Promise<unknown> {
		this.logVerbose(`Event: before_provider_request`);
		const ctx = this.createContext();
	...
	async emitBeforeAgentStart(
		...
		this.logVerbose(`Event: before_agent_start`);
	...
	async emitResourcesDiscover(
		...
		this.logVerbose(`Event: resources_discover`);
		const ctx = this.createContext();

...
export interface CreateAgentSessionOptions {
	...
	cwd?: string;
	/** Global config directory. Default: ~/.rozsa/agent */
	agentDir?: string;
...
function getAttributionHeaders(
	...
	if (
		...
		return { "x-opencode-session": sessionId, "x-opencode-client": "rozsa" };
	}
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	const defaultActiveToolNames = ["read", "bash", "edit", "write", "subagent"];
	...
	if (model) {
		const ctxPref = settingsManager.getContextWindowPreference(model.provider, model.id);
		if (ctxPref && ctxPref !== model.contextWindow) {
			model = { ...model, contextWindow: ctxPref };
		}
	}

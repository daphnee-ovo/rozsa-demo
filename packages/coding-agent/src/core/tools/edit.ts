...
import { Box, Container, Spacer, Text } from "@earendil-works/rozsa-tui";
...
function buildEditCallComponent(
	...
	const body =
		"error" in component.preview
			...
			: renderDiff(component.preview.diff, { filePath });
...
export function createEditToolDefinition(
	...
	return {
		...
		async execute(_toolCallId, input: EditToolInput, signal?: AbortSignal, _onUpdate?, _ctx?) {
			...
			// 干跑模式：读取文件并生成 diff 预览，但不写入
			if (dryRun) {
				const buffer = await ops.readFile(absolutePath);
				const rawContent = buffer.toString("utf-8");
				const { bom: _bom, text: content } = stripBom(rawContent);
				const normalizedContent = normalizeToLF(content);
				const { baseContent, newContent } = applyEditsToNormalizedContent(normalizedContent, edits, path);
				const diffResult = generateDiffString(baseContent, newContent);
				const patch = generateUnifiedPatch(path, baseContent, newContent);
				return {
					content: [{ type: "text", text: `[DRY-RUN] Would edit ${path}:\n${patch}` }],

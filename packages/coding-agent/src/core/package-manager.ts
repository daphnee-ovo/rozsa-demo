...
function isOfflineModeEnabled(): boolean {
	const value = process.env.ROZSA_OFFLINE;
...
type SkillDiscoveryMode = "rozsa" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "rozsa" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readRozsaManifestFile(packageJsonPath: string): RozsaManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { rozsa?: RozsaManifest };
		return pkg.rozsa ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readRozsaManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "rozsa");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private collectPackageResources(
		...
		const manifest = this.readRozsaManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof RozsaManifest];
	...
	private readRozsaManifest(packageRoot: string): RozsaManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		// Project skills from .rozsa/
		addResources(
			"skills",
			collectAutoSkillEntries(projectDirs.skills, "rozsa"),
			projectMetadata,
		...
		// User extensions from ~/.rozsa/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.rozsa/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "rozsa"),
			userMetadata,

    (function() {
      ...
      function parseSkillBlock(text) {
        // 新格式: <skill>\n<name>...</name>\n<content>\n...\n</content>\n<base_dir>...</base_dir>\n</skill>
        const match = text.match(/^<skill>\n<name>([^<]+)<\/name>\n<content>\n([\s\S]*?)\n<\/content>\n<base_dir>([^<]+)<\/base_dir>\n<\/skill>(?:\n\n([\s\S]+))?$/);
        if (match) {
          return { name: match[1], baseDir: match[3], content: match[2], userMessage: match[4]?.trim() || undefined };
        }
        // 兼容旧格式
        const legacy = text.match(/^<skill name="([^"]+)" (?:base-dir|location)="([^"]+)">\n([\s\S]*?)\n<\/skill>(?:\n\n([\s\S]+))?$/);
        if (!legacy) return null;
        const baseDir = legacy[2].endsWith('.md') ? legacy[2].replace(/\/[^/]+$/, '') : legacy[2];
        return { name: legacy[1], baseDir: baseDir, content: legacy[3], userMessage: legacy[4]?.trim() || undefined };
      }
